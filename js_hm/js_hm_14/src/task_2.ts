if (confirm('Почати тестування?')) {
	...
	const totalSumNewPrice = dataList.reduce((prevTotalSum, price) => prevTotalSum + price.price, 0)

	console.log(totalSumNewPrice);

	// 2)Знайти кількість товарів, у яких ціна зменшилась (price < old_price).

	const oldPriceNumber = dataList.filter(el => el.price < el.old_price).length
	console.log(oldPriceNumber);

	// 3) Товари, які доступні (sell_status:"available")

	const available = dataList.filter(el => el.sell_status === "available")
	...
	const newDataList = dataList.filter(({ sell_status }) =>
		sell_status === "available").map(({ id, price, old_price, usd_price }) => ({ id, price, old_price, usd_price }))
