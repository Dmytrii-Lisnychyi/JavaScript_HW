if (confirm('Почати тестування?')) {
	...
	class TMoney {
		...
		withdrawMoney(val: number) {
			...
			if (this.balance < 0 || this.balance < toDollar) throw new Error('You do not have money')

			this.balance -= toDollar
		}

		getRateForPlus100(): number {
			if (this.balance <= 0) throw new Error("You do not have money")
